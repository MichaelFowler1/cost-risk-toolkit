...
import cost_core.reporting.lot_workbook as W
...
def plain_book(tmp_path_factory, analogy_df, estimate_df):
    ...
    proj, ctx = run_lot_cost_model(analogy_df, estimate_df)
    W.save_complete_excel_workbook(
        str(path),
        proj,
        generate_analyst_summary(ctx, {"Program": "TEST"}),
        generate_fit_chart_data(ctx),
    )
    return path


@pytest.fixture(scope="module")
def risk_book(tmp_path_factory, analogy_df, estimate_df):
    """A workbook including the risk sheets.

    The three risk frames are built the way the desktop tool's risk bridge
    builds them, from the enrichment layer directly.
    """
    ...
    intervals = projection_intervals(ctx, proj, model, level=0.80).copy()
    # The priced lots come back without their unit ranges. They are
    # row-aligned with the projections, so carry them across: the band chart
    # plots against cumulative units.
    for col in ("First Unit in Lot", "Last Unit in Lot", "Complexity Factor"):
        if col in proj.columns and col not in intervals.columns:
            intervals[col] = proj[col].to_numpy()

    buy = simulate_buy(
        ctx, proj, model, n_iter=4000, seed=11, lot_correlation=0.30
    )
    risk_summary = pd.DataFrame(
        [
            ("Model", model),
            ("Simulated P50 ($)", f"{buy.p50:,.2f}"),
            ("Simulated P80 ($)", f"{buy.p80:,.2f}"),
            ("Simulated P90 ($)", f"{buy.p90:,.2f}"),
        ],
        columns=["Item", "Value"],
    )
    pct = np.arange(1, 100)
    risk_scurve = pd.DataFrame(
        {
            "Percentile": pct / 100.0,
            "Buy Total ($)": np.round(np.percentile(buy.totals, pct), 2),
        }
    )
    W.save_complete_excel_workbook(
        str(path),
    ...
    def test_anchors_are_spaced_wider_than_the_chart(self):
        ...
        cols = [
            openpyxl.utils.column_index_from_string(
                re.match(r"([A-Z]+)", W._chart_anchor(i, width)).group(1)
            )
            for i in range(3)
        ]
        step_cm = (cols[1] - cols[0]) * W._COL_CM
...
class TestSCurveSheet:
    ...
    def test_marker_names_carry_the_cost(self, risk_book):
        ...
        for name_col, cost_col in ((4, 5), (6, 7)):
            ...
            assert "$" in label
            assert label.split()[-1] == W._money_short(cost)

    def test_money_short_picks_a_sensible_unit(self):
        assert W._money_short(250_000_000) == "$250.0M"
        assert W._money_short(2_500_000_000) == "$2.5B"
        assert W._money_short(45_200) == "$45.2K"
        assert W._money_short(870) == "$870"
